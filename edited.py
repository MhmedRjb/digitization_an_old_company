...
###########################################
# ///''
# profit_df.columns = ['W', 'clint_number', 'clint_name', 'invoice_number', 'date',
#                      'invoice_type', 'item', 'barcode', 'value', 'cost', 'profit', 'profit/sellvalue',
#                      'profit/cost', "Unnamed: 13", "Unnamed: 14", "Unnamed: 15"]


# def drop_unwwated(coulmn_name, unwated_word):
#     profit_df.drop(profit_df.index[(profit_df[coulmn_name] == unwated_word)], axis=0, inplace=True)


# profit_df.index = range(1, len(profit_df) + 1)


# def all_down(df, column):
#     h = 0
...
# all_down(profit_df, 'invoice_number')
# all_down(profit_df, 'clint_number')
# all_down(profit_df, 'invoice_type')
# all_down(profit_df, 'date')
# all_down(profit_df, 'invoice_type')
# all_down(profit_df, 'clint_name')
# profit_df = profit_df.dropna(subset=['item']).reset_index(drop=True)
# profit_df = profit_df.dropna(axis='columns')
# profit_df = profit_df.groupby(['date', 'clint_name', 'clint_number', 'invoice_number', 'invoice_type', 'item'],
#                               as_index=False).agg(
#     {'value': 'sum', 'cost': 'sum', 'profit': 'sum', 'profit/sellvalue': 'mean', 'profit/cost': 'mean'})
# profit_df['invoice_number'] = profit_df['invoice_number'].astype('int32')
# profit_df["primary_key"] = profit_df['invoice_number'].astype(str) + "-" + profit_df["item"].astype(str) + "-" + \
#                            profit_df["invoice_type"].astype(str)
# profit_df['tax'] = np.where(profit_df['clint_number'].astype(str).str.contains('123101.*'), 0.08,
#                             np.where(profit_df['clint_number'].astype(str).str.contains('123104.*'), 0.04,
#                                      np.where(profit_df['clint_number'].astype(str).str.contains('123102.*'), 0.01,
#                                               0.00)))

# profit_df['profit_net'] = profit_df['profit'] - profit_df['profit'] * profit_df['tax']
# profit_df['value_net'] = profit_df['value'] - profit_df['value'] * profit_df['tax']

###########################
# accstat_df = pd.read_excel(r"D:\New folder (2)\account statement.xlsx")
# monyflow_df = pd.read_excel(r"D:\New folder (2)\account statement.xlsx")
# accstat_df.columns = ['Text139', 'acc_cd', 'RACC', 'date', 'bal_D', 'bal_c', 'invoice_number', 'invoice_type', 'mov_d',
#                       'mov_c', 'item', 'sqty', 'price', 'Text184', 'typo']

# accstat_df = accstat_df.dropna(subset=['item', 'sqty', 'typo']).reset_index(drop=True)
# accstat_df = accstat_df.dropna(axis='columns').reset_index(drop=True)
# accstat_df['total'] = accstat_df['mov_d'] - accstat_df['mov_c']
# # df['invoice_number'] =  df['invoice_number'].astype('int32')
# accstat_df = accstat_df.groupby(['date', 'invoice_number', 'invoice_type', 'item', 'typo'], as_index=False).agg(
#     {'total': 'sum', 'sqty': 'sum', 'price': 'mean'})
# accstat_df["primary_key"] = accstat_df['invoice_number'].astype(str) + "-" + accstat_df["item"].astype(str) + "-" + \
#                             accstat_df["invoice_type"].astype(str)
# accstat_df['total_sqty'] = np.where(accstat_df['invoice_type'].astype(str).str.contains('بيع آجل'), accstat_df['sqty'],
#                                     -accstat_df['sqty'])

# accstat_df.to_excel('NEW_2.xlsx')
